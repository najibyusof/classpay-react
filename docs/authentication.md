...
Reset a password using the token delivered through the password-reset email. This is the unauthenticated flow opened from the mobile login screen.
...
| `email`                 | string | required; valid account email        |
| `password`              | string | required; minimum 8 characters       |
| `password_confirmation` | string | required; must match `password`      |
...
The mobile app displays a success confirmation and returns the user to Login. The reset token is single-use and must not be stored after the request.

### Error responses

| Status | Cause                                    |
| ------ | ---------------------------------------- |
| 401    | Missing or invalid reset credentials     |
| 403    | Reset request is not permitted           |
| 422    | Invalid token, email, or password fields |
