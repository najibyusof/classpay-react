...
import type { StudentClass, StudentOrganization } from '../../types/student';
...
export function OrganizationClassesScreen({
  ...
  const classes = (classesQuery.data?.pages.flatMap((page) => page.data) ?? []).filter(
    (classItem) => String(classItem.organization_id) === String(organization.id),
  );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {classes.length === 0 ? (
        <EmptyState
          title="Tiada kelas"
          description={`Tiada kelas dalam ${organization.name}.`}
        />
      ) : (
